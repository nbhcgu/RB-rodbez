...
class CusRideStartPage extends StatefulWidget {
  static const String idScreen = "SearchPage";
  CusRideStartPage({required this.searchList, required this.status_type});
  var searchList;
  final int status_type;
  @override
  CusRideStartPageState createState() => CusRideStartPageState();
}

class CusRideStartPageState extends State<CusRideStartPage>
...
//crear Marker
 ...
 getTrack() async {
    ...
    DatabaseReference db = FirebaseDatabase.instance
        ...
        .child('users_loc')
        .child("uid" + id);
    DataSnapshot event = await db.get();
    getDB(event);
                            ...
                            children: [
                              Row(children: [
                                Expanded(
                                  ...
                                  SizedBox(
                                    child: details["driver_details"]["driver_icon"].toString().isEmpty? Icon(Icons.account_circle_rounded,color: Colors.black,size: 50,):
                                    CircleAvatar(radius: 25,backgroundImage: NetworkImage(details["driver_details"]["driver_icon"].toString()),),),),
                                ),
                                Text(details["driver_details"]["driver_name"].toString(), style: TextStyle(color: Colors.black,fontSize: 16,fontWeight: FontWeight.bold),)
                              ],),
                              Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () async{
                                if(indexChk<2){
                                  var mapSchema = 'https://www.google.com/maps/dir/?api=1&origin=${dlat},${dlan}&destination=${ulat},${ulan}&travelmode=driving&dir_action=navigate';
                                  ...
                                  var mapSchema = 'https://www.google.com/maps/dir/?api=1&origin=${dlat},${dlan}&destination=${details["destination_lat"]},${details["destination_long"]}&travelmode=driving&dir_action=navigate';
                                  if (await canLaunchUrl(Uri.parse(mapSchema))) {
                                    await launchUrl(Uri.parse(mapSchema));
                                  } else {
                                    throw 'Could not launch $mapSchema';
