...
class CustomerCompletePage extends StatefulWidget
{
...
class _ArrivedPageState extends State<CustomerCompletePage> {
  String id="",rideId="",sPlace="",ePlace="",date="",cab="",cab_icon="",cabData="",cabSeat="",fare="",baseFare="",taxes="",status="",
      msg="",serEx="",serExImg="",serExNo="",fare_caption="",extra_km_fare="",extra_time_charge="",round_off_amount="",tax_text="",fare_heading="",base_fare_text="",
  ...
  sendInv() async {
    ...
    print("user_id-" + idu);
    print("user_login_token-" + idL);
    API.sendInv(idu, idL, widget.id,emailTextEditingController.text.toString()).then((response) {
      EasyLoading.dismiss();
      setState(() {
        print(response.body);
        var jVar = jsonDecode(response.body);
        print(jVar["data"]);
        if (jVar["status"] == 200) {
          var data = jVar["data"]["message"].toString();
  ...
  getRiderSelf() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    setState((){
      emailTextEditingController.text = prefs.getString('user_email') ?? "";
    });
  }
          ...
          children: [
           ...
           is_review? GestureDetector(
              ...
              bool chk = await  Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) =>
                            ReviewPage(id: id,rideId:rideId,user_type: "user",type: 0,)));
              if(chk!=null){
                if(chk){
                  setState((){
                    is_review = false;
