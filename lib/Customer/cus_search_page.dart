...
import 'package:rodbez/Customer/cus_cab_select_page.dart';
...
class CustomerSearchPage extends StatefulWidget {
  CustomerSearchPage({required this.cur_address,required this.cur_lat,required this.cur_long});
...
class CustomerSearchPageState extends State<CustomerSearchPage> {
  ...
  late String origin_place="", origin_lat="", origin_long="", destination_place="", destination_lat="", destination_long="",
      origin_id="", destination_id="",location_type="db";
  @override
  void initState() {
    ...
    setState(() {
      _startSearchFieldController.text = widget.cur_address;
    });
    origin_place = widget.cur_address;
    origin_lat = widget.cur_lat;
    origin_long = widget.cur_long;
    origin_id = "0";location_type = "db";
  }
  ...
  selfSearch() async {
    if(origin_id.isEmpty || destination_id.isEmpty){
     ...
     EasyLoading.show();
      ...
      print("user_id-" + id);
      print("user_login_token-" + idL);
      API.selfSearch(id, idL, origin_place,origin_lat,origin_long,destination_place,destination_lat,
      destination_long,origin_id,destination_id,location_type).then((response) {
        EasyLoading.dismiss();
        print(response.body);
          var jVar = jsonDecode(response.body);
          print(jVar);
          if (jVar["status"] == 200) {
            if(json.decode(response.body)['chk']==1) {
              searchList = json.decode(response.body)['data']['cab_list'];
              var exclusive_offers = json.decode(response.body)['data']['exclusive_offers'];
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) =>
                          CustomerCabSelectPage(searchList: searchList,exclusive_offers: exclusive_offers,
                            start_lat: origin_lat,
                            start_long: origin_long,
                            start_address: origin_place,
                            end_lat: destination_lat,
                            end_long: destination_long,
                            end_address: destination_place,)));
            }else{
              ...
              Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (context) =>
                          CustomerServiceNotPage(message: dt["message"],
                            caption: dt["caption"], start_lat: origin_lat,
                              start_long: origin_long,
                              start_address: origin_place,
                              end_lat: destination_lat,
                              end_long: destination_long,
  ...
  Widget _getBody() {
    return Stack(children: [
      ...
      Container(
        margin: EdgeInsets.only(top:270,bottom: 100),
